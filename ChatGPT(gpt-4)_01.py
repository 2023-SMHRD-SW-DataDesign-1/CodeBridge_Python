...
# LangChain의 대화Chain 라이브러리
from langchain.chains import ConversationChain

# LangChain의 대화 내용 저장하는 Memory 라이브러리
...
def test2():
    ...
    # 객체 생성하고 옵션 설정하기
    llm = ChatOpenAI(temperature=1.0,  # temperature : 창의성 (0.0 ~ 2.0) 사이로 설정 가능.
                     max_tokens=800,  # max_tokens : 최대 토큰수
                     model_name='gpt-4',  # model_name : 모델명
                     streaming=True,  # streaming : 실시간으로 답변받기
                     callbacks=[StreamingStdOutCallbackHandler()]
    ...
    # Memory객체 memory변수에 담기
    memory = ConversationBufferMemory()

    # 대화Chain객체 conversation변수에 담기
    ...
    AI_response = conversation.predict(input='''
                ...
                예시에 맞춰 문제를 출제해주세요. 그리고 이전에 출제했던 문제와는 다른 새로운 유형의 문제를 출제해주세요.
    ...
    # 스트리밍으로 답변 출력하기
    # response = llm.predict(AI_response)
