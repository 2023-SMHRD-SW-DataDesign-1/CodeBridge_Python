...
### 스프링에서 받은 Json데이터에서 조건의 갯수가 딱 3개로 정해져있는 경우 => 원하는 결과만 파싱해서 return 가능!
# LangChain의 PromptTemplate 사용O
...
def test1():
    ...
    print("학생이 제출한 코드 : " + code, "/", "조건1번 : " + condition_1, "/", "조건2번 : " + condition_2, "/",
          "조건3번 : " + condition_3)
    ...
    def getbard():
        cookie_dict = {
            ...
            "__Secure-1PSIDTS": "sidts-CjEB3e41hb37IfP3Lgt_hIEh4Jp6hwHfPEHQg187gQ0P1ZjPIQX5AkiEtIf-SRlNjGvlEAA", }  # 매번 바뀌므로 주의
        ...
        multiple_input_prompt = PromptTemplate(
            input_variables=['code', 'result', 'condition1', 'condition2', 'condition3'],
            template="""
                                                       '{code}'
        ...
        print(bard.get_answer(
            multiple_input_prompt.format(code=code, result="code의 실행 결과값 전부 출력",
                                         condition1=f"{condition_1}조건에 부합하면 <통과> 출력, 부합하지 않으면 <실패> 출력",
                                         condition2=f"{condition_2}조건에 부합하면 <통과> 출력, 부합하지 않으면 <실패> 출력",
                                         condition3=f"{condition_3}조건에 부합하면 <통과-end> 출력, 부합하지 않으면 <실패-end> 출력"))[
                  'content'])
